...
"""
The goal of the program is to calculate the mean log likelihood of a dataset B on dataset A.
The model is a mixture of gaussians where each gaussian component is centered around a data point in A.
...
Finally the testing dataset's mean log likelihood is calculated on the training. 
"""
...
def mnist():
    """ Load MNIST data. Optimize the standard deviation. Calculate mean log likelihood of the mnist testing dataset. 
    ...
    print("Best std dev: " + str(best_std_dev) + " achieved with a mean log likelihood of: " + str(best_log_probs))
    t0 = time.time()
    mean_log_prob = kde_mg.mean_log_prob_tiling_multi_proc(mnist_train, mnist_test, best_std_dev)
    total_time = time.time() - t0
    print("Calculating mean log likelihood of test mnist dataset...")
    print("Running time: " + str(total_time) + " seconds")
    print("Mean log likelihood on mnist test data: " + str(mean_log_prob))


def cifar100():
    """ Load CIFAR100 data. Optimize the standard deviation. Calculate mean log likelihood of the CIFAR100 testing 
    ...
    total_time = time.time() - t0
    print("Calculating mean log likelihood of test CIFAR100 dataset...")
    print("Running time: " + str(total_time) + " seconds")
    print("Mean log likelihood on CIFAR100 test data: " + str(mean_log_prob))


def find_optimal_value(train, val, dataset_name):
    """ For each std dev calculate mean log likelihood for validation on training. Select std dev with highest 
    log likelihood."""
...
def visualization_of_optimal_value(std_devs, mean_log_probs, dataset_name):
    """ Visualize optimal value with a plot of the different log likelihoods for each std dev."""
    plt.figure()
