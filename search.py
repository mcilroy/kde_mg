...
def mnist():
    ...
    t0 = time.time()
    mean_log_prob = kde_mg.mean_log_prob(mnist_train[:200], mnist_test, best_std_dev)
...
def cifar100():
    ...
    print("Best std dev: " + str(best_std_dev) + " achieved with a mean log probability of: " + str(best_log_probs))
    t0 = time.time()
    mean_log_prob = kde_mg.mean_log_prob(cifar100_train, cifar100_test, best_std_dev)
    total_time = time.time() - t0
